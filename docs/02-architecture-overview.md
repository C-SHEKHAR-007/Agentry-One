...
```mermaid
flowchart TD
    UI["React (Vite) UI<br>Agent list, job submit, progress, artifact viewer"]
    API["Fastify API (Node)<br>Registry, Workflow, Job/Queue, Artifact modules"]
    PG["PostgreSQL<br>System of record (see doc 05)"]
    REDIS["Redis (BullMQ)<br>Durable job queues"]
    WV["worker-video (Python)<br>Runs VSplitter pipeline.analyze() / render_selected()"]
    WS["worker-sketch (Python)<br>Runs SD-Turbo diffusers pipeline"]
    FS["Local Filesystem Artifact Storage<br>Shared volume: transcripts, clips, images, manifests"]
...
*Note: `worker-video` also reaches a host-installed Ollama server (targeted network path — see doc 08 and doc 10) for LLM-based highlight re-ranking, exactly as VSplitter does today.*
