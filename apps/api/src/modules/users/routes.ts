...
async function activeOwnerCount(): Promise<number> {
  return prisma.user.count({ where: { role: "owner", email: { not: "local@agentry.dev" } } });
}
...
export async function usersRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { email: string; firstName?: string; lastName?: string; avatarUrl?: string; password: string; role?: string } }>(
    "/users",
    async (req, reply) => {
      if (!requireOwner(req, reply)) return;
      const { email, firstName, lastName, avatarUrl, password, role } = req.body;
  ...
  app.patch<{ Params: { id: string }; Body: { firstName?: string; lastName?: string; avatarUrl?: string; role?: string; password?: string } }>(
    "/users/:id",
    async (req, reply) => {
      if (!requireOwner(req, reply)) return;
      ...
      const { firstName, lastName, avatarUrl, role, password } = req.body;
      if (role && role !== "owner" && role !== "member") {
        return reply.code(400).send({ error: "role must be 'owner' or 'member'" });
      }
      if (
        role === "member" &&
        target.role === "owner" &&
        target.email !== "local@agentry.dev" &&
        (await activeOwnerCount()) <= 1
      ) {
  ...
  app.delete<{ Params: { id: string } }>("/users/:id", async (req, reply) => {
    ...
    if (target.role === "owner" && target.email !== "local@agentry.dev" && (await activeOwnerCount()) <= 1) {
