...
export async function contentBriefsRoutes(app: FastifyInstance) {
  app.post<{ Params: { id: string }; Body: { topic: string; tone?: string; formats: string[] } }>(
    "/projects/:id/briefs",
    async (req, reply) => {
      try {
        const result = await createContentBriefTemplate(req.params.id, req.body.topic, req.body.tone, req.body.formats);
